...
type InitData = {
  ...
  colums: { [columnId: string | number]: ColumnType };
};
...
const Main = ({ FileInput, dataService, authService }: MainProps) => {
  ...
  const [initData, setInitData] = useState<InitData>({ items: {}, colums: {} });
  ...
  const onSubmitItem = useCallback(
    (item: ItemType) => {
      const itemUpdated = { ...initData.items, item };
      const columnUpdated = initData.colums
        ? [...initData.colums, item.id]
        : [item.id];
      setOnDialog(false);
      setType("");
