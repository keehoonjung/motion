...
const Main = ({ dataService, authService }: MainProps) => {
  ...
  const onSubmitItem = (item: ItemType) => {
    const updated = [...items];
    updated.push(item);
    const updateIndex = updated.indexOf(item);
    console.log(updateIndex);

    setItem(updated);
    setOnDialog(false);
    setType("");
    dataService.writeData(userId, updated, updateIndex);
  };

  const onDeleteItem = (item: ItemType) => {
    const updateIndex = items.indexOf(item);
    setItem((items) => {
      const updated = [...items];
    ...
    dataService.deleteData(userId, updateIndex);
  };
