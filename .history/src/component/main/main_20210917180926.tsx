...
export type ItemArrayState = ItemType[];
...
const Main = ({ dataService, authService }: MainProps) => {
  ...
  const [itemsArray, setItemsArray] = useState<ItemArrayState>([]);
  ...
  useEffect(() => {
    ...
    dataService.readData(userId, (items: ItemState) => {
      setItem(items);
      const newItem = Object.values(items);
  ...
  const moveItem = useCallback((dragIndex: number, hoberIndex: number) => {
    const dragItem = itemsArray[dragIndex];
    setItemsArray(
      update(itemsArray, {
        $splice: [
          [dragIndex, 1],
          [hoberIndex, 0, dragItem],
        ],
      })
    );
  });
