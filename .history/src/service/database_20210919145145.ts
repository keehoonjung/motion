...
export interface DataInterface {
  writeData(userId: string, item: ItemType, index: number): void;
...
export class DataSerivce implements DataInterface {
  ...
  writeData(userId: string, item: ItemType, index: number) {
    set(ref(this.database, `${userId}/items/${index}`), item);
  }
