...
export interface DataInterface {
  ...
  deleteData(userId: string, id: string | number, index: number): void;
}
...
export class DataSerivce implements DataInterface {
  ...
  deleteData(userId: string, id: string | number, index: number) {
    remove(ref(this.database, `${userId}/items/${id}`));
    remove(ref(this.database, `${userId}/columns/column1/itemIds/${index}`));
  }
}
